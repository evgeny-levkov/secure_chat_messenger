...
class MessageRequest:
    ...
    def to_dict(self) -> dict[str, str]:
        return {'status': 'send_message', 'message': self.message,'recipient': self.recipient_id, 'sender_name': self.sender_name, 
                'encryption': self.encryption, 'encryption_key': self.encryption_key}

    @classmethod
    def from_dict(cls, model: dict[str, str | int]) -> MessageRequest | None:
        try:
            return cls(model['message'], model['recipient'], model['sender_name'], model.get('encryption', None), model.get('encryption_key', None))
